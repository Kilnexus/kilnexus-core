...
const zip_extractor = @import("zip_extractor.zig");
...
fn extractTarXz(
    ...
    try std.tar.pipeToFileSystem(out_dir, &tar_reader, .{
        .strip_components = @intCast(strip_components),
    });
}
...
fn extractZip(
    ...
    try zip_extractor.extractZipFile(allocator, archive_path, install_dir, strip_components);
}
